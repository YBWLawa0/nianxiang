...
“念想”是一个让用户随手记录、随心分享的 AI 情感向产品。用户可以记录随笔，阿响会以亲近朋友的视角给出能量刻度、九宫格标签和亲切回应；用户也可以把当天随笔生成一篇以自己视角写成的日记。
...
### 7. emotional.md 情感记忆系统（创新亮点）

我们创新性地引入了 **emotional.md** 情感记忆文档机制——为每位用户维护一份持续生长的"情感记忆文件"。系统会将用户历史对话中的关键情绪事件、重要时刻和情感模式自动提取并写入 emotional.md，使 AI 伴侣"阿响"能够：

- **跨会话记忆**：记住用户过去分享的重要经历和情绪转折点
- **情感模式识别**：发现用户长期的情绪规律（如"每到周五就容易焦虑"）
- **深度个性化**：基于情感记忆生成更有温度、更懂用户的回应和日记
- **记忆向量检索**：通过向量数据库实现语义级别的情感记忆检索

emotional.md 是念想区别于普通日记产品的核心创新，让 AI 从"工具"进化为"懂你的知己"。
...
├─────────────────────────────────────────────────────────┤
│  数据存储层                                               │
│  • 主数据库（MySQL）    • 向量数据库（情感记忆检索）        │
│  • JWT 认证                                              │
└─────────────────────────────────────────────────────────┘
```

> **OpenClaw** 是念想的 AI 引擎核心架构，负责情感分析、日记生成、情感记忆管理等所有 AI 能力的编排与调度。
